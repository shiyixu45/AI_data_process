    ...
    with open(output_path, 'w', encoding='utf-8', buffering=8*1024*1024) as f:
        for i in range(num_rows):
            row = {
                ...
                'text': ''.join(random.choices(string.ascii_letters + ' ', k=random.randint(50, 500))),
...
def run_performance_test(input_path: str, output_dir: str, plugin_name: str,
                         workers: int, batch_size: int, keep_order: bool):
    """运行性能测试"""
    ...
    plugin_dir = str(Path(__file__).parent.parent / "src" / "plugins")
